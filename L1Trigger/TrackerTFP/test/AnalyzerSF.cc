...
namespace trackerTFP {
  ...
  /*! \class  trackerTFP::AnalyzerSF
   *  \brief  Class to analyze hardware like structured TTStub Collection generated by Seed Filter
   *  \author Thomas Schuh
   *  \date   2020, Apr
   */
  class AnalyzerSF : public one::EDAnalyzer<one::WatchRuns, one::SharedResources> {
  public:
    AnalyzerSF(const ParameterSet& iConfig);
  ...
  AnalyzerSF::AnalyzerSF(const ParameterSet& iConfig) : useMCTruth_(iConfig.getParameter<bool>("UseMCTruth")), nEvents_(0) {
  ...
  void AnalyzerSF::beginRun(const Run& iEvent, const EventSetup& iSetup) {
  ...
  //
  void AnalyzerSF::formTracks(const TTDTC::Stream& stream, vector<vector<TTStubRef>>& tracks) const {
    vector<StubSF> stubs;
  ...
  //
  void AnalyzerSF::associate(const vector<vector<TTStubRef>>& tracks, const StubAssociation* ass, set<TPPtr>& tps, int& sum) const {
...
DEFINE_FWK_MODULE(trackerTFP::AnalyzerSF);